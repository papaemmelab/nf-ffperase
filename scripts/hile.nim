## Adaptation of hileup by brentp
## Source: https://github.com/brentp/hileup
...
proc hileup*(bam:Bam, chrom: string, position: int, reference: Fai, cfg: Config, snv: bool, result: var Hile) {.inline.} =
  ...
  for aln in bam.query(chrom, position, position + 1):
    ...
    for op in aln.cigar:
      if r_off == position + 1 and not skip_last and not snv:
...
proc hileup*(bam:Bam, chrom: string, position: int, reference: Fai, cfg: Config, snv: bool): Hile =
    hileup(bam, chrom, position, reference, cfg, snv, result)
...
when isMainModule:
  ...
  for i in 20100..20100:
    echo i
    var h = b.hileup("1", i, fai, cfg, True)
    echo h
